...
- 调整位置 = 决定你的插件内容在灵动岛上排在左边还是右边。位置与三个原生模块（时间日期 / 硬件占用 / 媒体控制器）**共用同一张顺序表**，列表卡片顶部那行「顺序：…」会把当前**真正显示在岛上**的内容按左右次序摊开，行内的 `#N` 就是它在这份显示清单里的第几个。**被禁用 / 加载失败的插件、没有勾选显示的原生模块不参与排序**：不会出现在「顺序：…」里、也不计入 `#N`，但它们在顺序表里的位置会一直保留着，重新启用 / 重新勾选显示后会自己插回原来的位置。无论是否开启组合模式，调整都立即生效，并且会记到下次启动。
