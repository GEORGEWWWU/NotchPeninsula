...
- 调整位置 = 决定你的插件内容在灵动岛上排在左边还是右边。位置与三个原生模块（时间日期 / 硬件占用 / 媒体控制器）**共用同一张顺序表**，所以「插件中心」里显示的 `#N` 是这张表里的次序，不一定等于你在岛上肉眼数出来的第几个；列表卡片顶部那行「顺序：…」会把整张表摊开给你看。无论是否开启组合模式，调整都立即生效，并且会记到下次启动。
