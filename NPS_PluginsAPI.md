...
- **右键展开 / 收起**：右键组件展开详情页；详情页展开时再在岛内右键一次就收起（不会再弹设置窗口）。
  注意岛内的右键是**分层消费**的，顺序为：收起详情页 → 展开原生媒体面板 → 广播给命中的组件 → 打开设置窗口。
  其中「展开原生媒体面板」指的是媒体控制激活时右键**折叠态的媒体标题文字**（原生区域的右键快捷方式，给直接交互模式用）；
  这个快捷方式**只在插件行真的有组件显示时才生效**——纯媒体控制器（没有插件组件）时它会被关掉，右键照旧直接打开设置窗口，
  免得整条岛体都是标题热区、把右键整片吃掉。
  它命中的是原生媒体标题那一段文字，而插件组件占的是岛体右侧**独立的预留区**，两者几何上不重叠，
  所以正常情况下插件组件的右键不会被它抢走；但如果你把组件画到媒体标题那一段上（宿主不推荐），右键会先被原生逻辑吃掉。
