...
- 岛内右键：**按区域直达设置页签** —— 媒体控制器区域（标题 / 歌词 / 频谱 / 播放按钮 / 空白）打开「媒体设置」，
...
- 协议细节见WIKI中的文档
- 支持使用服务端推送的实时频谱
- 内置音量只有一个下游：**接了 Just Solo 的 WS 就调播放器音量**（协议 v1.3.0 的 `volume` 指令），没接才调系统主音量
- Just Solo 侧（界面 / 快捷键 / 其它客户端）改音量会回推并落进内置音量；连接时补推的初始值不参与同步
