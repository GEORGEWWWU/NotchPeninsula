...
- 插件窗口支持**双向文件拖放**：可从资源管理器拖入文件 / 文件夹（拖动过程中有悬停回调，可做高亮与落点提示），也可把条目拖出到桌面或资源管理器，适合做「文件中转站」这类工具
- 插件**详情页**同样支持拖放：右键展开的插件面板能直接接收拖入的文件，也能把内容拖出去（面板里带完整的鼠标按下 / 移动 / 抬起事件）
- 组件可声明「**收起态也收文件**」：用户把文件直接拖到岛上那个还没展开的组件图标上，详情页会自动展开并收下文件，不必先点开面板
- 有文件正被拖着经过岛体时**穿透模式临时失效**：岛体不再悬停淡出，否则它一淡到全透明就会从 OLE 的拖放命中测试里消失，文件再也放不进来；拖放结束自动恢复
...
- **个性化中心**：主题、背景不透明度、刘海样式与圆角、各状态尺寸、DPI 缩放（其中「待机显示 → 水平宽度」「消息通知 → 弹出的宽度」为系统自动调整，不开放手动设置）
...
- **交互设置**：自动隐藏（总开关「允许灵动岛自动隐藏」，需先开启它才能选择下方三种模式；「焦点离开时 / 暂停播放后 / 全屏时」三种模式互相独立、可任意组合）、媒体交互方式、鼠标穿透
